...
extern "C"
{
    ...
    EM_JS(void, himg_add_js_hook_clipboard, (void), {
        ...
        Module.sokol_paste = function(event) {
            ...
            ccall('_himg_emsc_onpaste', 'void', ['string'], [pasted_str]);
        };
        // console.log("himg_add_js_hook_clipboard 4");
        window.addEventListener('paste', Module.sokol_paste);
    });

    EM_JS(void, himg_remove_js_hook_clipboard, (void), {
        window.removeEventListener('paste', Module.sokol_paste);
    });

    void EMSCRIPTEN_KEEPALIVE _himg_emsc_onpaste(const char *str)
    {
        // std::cout << "_himg_emsc_onpaste " << str << std::endl;
    ...
    EM_JS(void, himg_js_write_clipboard, (const char* c_str), {
